  ...
  with tempfile.TemporaryDirectory(prefix="repo-ci-") as temp_name:
    ...
    for step in env_config.get("steps", []):
      ...
      if rc == 2 or kind == "prerequisite":
        saw_incomplete = True
        # A prerequisite failure means the environment is not valid for later
        # gates.  An explicit exit 2 from a validation gate is infrastructure
        # evidence for that gate; unrelated validation gates may still run.
        if kind == "prerequisite":
          break
        continue
      saw_failure = True
      # Source-defined setup failures make dependent gates invalid, but are
      # genuine validation failures rather than infrastructure gaps.
      if kind == "setup":
        break
      # Independent validation gates continue so the initial RED surface is
      # not hidden by the first failing gate.
