...
A browser host should consume the committed generated artifact and pin it to an
exact AIConversationCore Git commit rather than `main`. Pinning prevents an
unrelated future Core change from altering a consumer without a corresponding
consumer revision and regression review.
...
- verifies the requested tag does not already exist;
